...
const ProjectGrid = ({ repos, onLoadMore, isLoading, showButton, activeFilter, onFilterChange, onTagClick }) => {
  return (
    <>
      ...
      {showButton && (
        <div className="text-center mt-4">
          <button
            onClick={onLoadMore}
            disabled={isLoading}
            className="bg-(--primary-button) hover:bg-(--primary-button-hover) px-6 py-2 rounded-md text-sm flex items-center gap-2 mx-auto disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <>
                Fetching...
